...
public class ActivityServiceImpl implements ActivityService {
        ...
        try {
            ...
            if (!ActivityStatus.EXAMINE.getStatus().equals(activityDto.getStatus())) {
                redisTemplate.opsForHash()
                        .put(RedisKey.ACTIVITY_HASH_KEY, RedisKey.ACTIVITY_HASH_FIELD + activityDto.getId(), activityDto);
            }
    ...
    public Result getActivityById (Long id) {
        ActivityDto activityDto = (ActivityDto) redisTemplate.opsForHash()
                .get(RedisKey.ACTIVITY_HASH_KEY, RedisKey.TAG_HASH_FIELD + id);
        if (activityDto == null) {
            Activity activity = activityMapper.selectById(id);
            if (activity == null) {
            ...
            redisTemplate.opsForHash()
                    .put(RedisKey.ACTIVITY_HASH_KEY, RedisKey.ACTIVITY_HASH_FIELD + activity.getId(), activityDto);
        }
        return new Result().success(activityDto);
    }
    ...
    public Result updateActivity (User user, ActivityDto activityDto, QuestionAndAnswer[] questionAndAnswers) {
        Activity activity = activityMapper.selectOne(new QueryWrapper<Activity>().eq("id", activityDto.getId())
