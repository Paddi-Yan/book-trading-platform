...
public class ActivityController {
    ...
    @ApiOperation("添加活动信息")
    public Result addActivity (ActivityDto activityDto, @RequestBody QuestionAndAnswer[] questionAndAnswers, @PathVariable(name = "userId", required = true) Long userId) {
    ...
    @ApiOperation("修改活动信息")
    public Result updateActivity (@PathVariable Long userId, @PathVariable Long id, ActivityDto activityDto, @RequestBody QuestionAndAnswer[] questionAndAnswers) {
        if (activityDto.getStartTime().after(activityDto.getDeadline())) {
            return new Result().fail(HttpStatusCode.ERROR).message("活动截止日期与活动开始日期参数错误!");
        }
        User user = userService.getUserById(Math.toIntExact(userId));
